...
def item():
    ...
    if id:
        try:
            result = DB.selectOne(
                "SELECT id, name, description, stock, cost, image FROM IS601_S_Items WHERE id = %s", id)
...
def items():
    ...
    try:
        result = DB.selectAll(
            "SELECT id, name, description, stock, cost, image FROM IS601_S_Items LIMIT 25",)
...
def shop_list():
    ...
    try:
        result = DB.selectAll(
            "SELECT id, name, description, stock, cost, image FROM IS601_S_Items WHERE stock > 0 LIMIT 25",)
        if result.status and result.rows:
            rows = result.rows
    except Exception as e:
        print("Error fetching items", e)
...
def cart():
    ...
    if id and user_id:
        if quantity > 0:
            try:
                result = DB.selectOne(
                    "SELECT cost,name from IS601_S_Items WHERE id = %s", id)
                print("result", result)
                if result.status and result.row:
                    ...
                    if item_id:  # update from cart
                        ...
                        if result.status:
                            flash(
                                f"Updated quantity for {name} to {quantity}", "success")
                    else:  # add from shop
                        result = DB.insertOne("""
                        ...
                        if result.status:
                            flash(
                                f"Added {quantity} of {name} to cart", "success")
            except Exception as e:
            ...
            try:
                result = DB.delete(
                    "DELETE FROM IS601_S_Cart where item_id = %s and user_id = %s", id, user_id)
                if result.status:
    ...
    try:
        ...
        for item in cart:
            if item["quantity"] > item["stock"]:
                flash(
                    f"Item {item['name']} doesn't have enough stock left", "warning")
                has_error = True
            if item["cart_cost"] != item["item_cost"]:
                flash(
                    f"Item {item['name']}'s price has changed, please refresh cart", "warning")
                has_error = True
        ...
        if not has_error:
            ...
            attrs = [("life", -1), ("speed", -2), ("fire_rate", -3),
                     ("damage", -4), ("radius", -5)]
        ...
        if not has_error:
            result = DB.delete(
                "DELETE FROM IS601_S_Cart WHERE user_id = %s", current_user.get_id())

        if not has_error:
            details = f"Spent {total} on {quantity} upgrades"  # TBD
            current_user.account.remove_points(-total,
                                               reason="purchase", details=details)
            DB.getDB().commit()
