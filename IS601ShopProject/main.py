...
def create_app(config_filename=''):
    ...
    with app.app_context():
        ...
        '''
        from auth.payment import payment
        app.register_blueprint(payment)
        '''
        # load the extension
        # must be defined/initialized for identity to work (flask_principal)
        ...
        def load_user(user_id):
            ...
            # note: we'd lose roles here since it makes a new user object without a roles query
            print("loading user from DB")
            try:
                result = DB.selectOne(
                    "SELECT id, email FROM IS601_Users WHERE id = %s", user_id)
                if result.status:
